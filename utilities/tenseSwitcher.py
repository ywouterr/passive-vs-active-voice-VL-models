...
from verb_inflector import verbPassifier, conv2pc

nlp = spacy.load('en_core_web_sm')

def presContinuer(doc, rec=False):
    """ 
    ...
    for sent in parse.sents:
            ...
            if processed[word.i] == False:
                ...
                if word.dep_ =='nsubj':
                    if not subj:
                        ...
                        subj_plural = word.tag_ in ('NNS','NNPS')
                        if word.text == 'I':
                            I = True
                        for w in word.subtree:
                            processed[w.i] = True
                ...
                if word.dep_ in ('xcomp','ccomp'):
                    if word.head.dep_ == 'ROOT':
                        ...
                        xcomp = presContinuer(xcomp, True).strip(' .')
        ...
        if subj == '' and (dobj == '' or pobj == ''):
            newdoc += str(sent) + ' '
            if rec == False:
                newdoc = 'not available'.upper()
                return newdoc
            return str(sent) 
        
        # change tense to pc
        pres_cont = conv2pc(verb, aux, subj_plural, I)
        aux = pres_cont[0]
        verb = pres_cont[1]
        ...
        if advcl:
            ...
            if list(advcl.subtree)[0].dep_ == 'mark':
                ...
                temp_advcl = ''.join(w.text_with_ws.lower() if w.tag_ not in ('NNP','NNPS') else w.text_with_ws for w in advcl.subtree).strip()
                advcl = ''
                for i in range(len(temp_advcl)):
                    if i >= len(marker):
                        advcl += temp_advcl[i]
            else:
                advcl = ''.join(w.text_with_ws.lower() if w.tag_ not in ('NNP','NNPS') else w.text_with_ws for w in advcl.subtree).strip()
            ...
            if contains_s and contains_o:
                advcl_pass = ', ' + marker + presContinuer(advcl, True).strip(' .')
            else:
        ...
        # pobj_front, after prep0
        newsent0 = ' '.join(list(filter(None, [prep0,subj,adverb['bef'],aux,verb,
            prep_verbmod,part,acomp,dobj,pobj_part,prep2,pobj,adverb['aft'],advcl_pass,coord,xcomp,prep1])))+punc
        newsent = ''
