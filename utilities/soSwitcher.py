...
from verb_inflector import *

nlp = spacy.load('en_core_web_sm')

def switcheroo(doc, rec=False):
    """ 
    ...
    for sent in parse.sents:
            ...
            if processed[word.i] == False:
                ...
                if word.dep_ in ('xcomp','ccomp'):
                    if word.head.dep_ == 'ROOT':
                        ...
                        xcomp = switcheroo(xcomp, True).strip(' .')
        ...
        if dobj:
            presCT = makeObjActive(verb, aux, aplural_d, I)
        else:
            presCT = makeObjActive(verb, aux, aplural_p, I)
        aux = presCT[0]
        verb = presCT[1]
        ...
        if advcl:
            ...
            if contains_s and contains_o:
                advcl_pass = ', ' + marker + switcheroo(advcl, True).strip(' .')
            else:
                advcl_pass = ', ' + marker + advcl
            
        pobj_front = ''
        if place_bef_v or not dobj:
            pobj_front = pobj
            pobj = ''

        
        newsent0 = ' '.join(list(filter(None, [prep0,pobj_front,dobj,adverb['bef'],aux,verb,
            prep_verbmod,part,acomp,subj,pobj_part,prep2,pobj,adverb['aft'],advcl_pass,coord,xcomp,prep1])))+punc
        newsent = ''
